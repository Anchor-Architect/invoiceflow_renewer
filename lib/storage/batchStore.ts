...
// Create an empty batch with a known total file count (used for chunked upload)
export const initBatch = (total: number): InternalBatchState => {
  const id = createId();
  ensureDataDir();
  fs.mkdirSync(batchDir(id), { recursive: true });
...
// Append a chunk of files to an existing batch (used for chunked upload)
export const addFilesToBatch = async (
  batchId: string,
  files: UploadedPdf[]
): Promise<InternalBatchState> => {
  const current = getBatch(batchId);
  if (!current) throw new Error("Batch not found");

  // Write new files to disk in parallel
  const newSourceFiles = await Promise.all(
    files.map(async (f) => {
      const safeName = path.basename(f.name).replace(/[^a-zA-Z0-9._-]/g, "_");
      const filePath = path.join(batchDir(batchId), `${f.id}-${safeName}`);
      await fs.promises.writeFile(filePath, f.buffer);
      return { id: f.id, name: f.name, filePath };
    })
  );

  current.files.push(...files.map((f) => ({ id: f.id, name: f.name })));
  current.sourceFiles.push(...newSourceFiles);
  writeState(current);
  return current;
};
